...
def setRTCEventFlag(flagbit, enabled):
    ...
    bus.write_byte_data(
        ADDR_RTC,
        1,
        (out & (0xFF - flagbit - disableflagbit - ti_tp_flag)) | enableflagbit,
    )
            ...
            if curdate >= 0:
                ...
                if curmonth >= 0:
                    ...
                    monthdatestr = (
                        ...
                        + " "
                        + str(curdate)
                        + getNumberSuffix(curdate)
                    )
                else:
                    monthdatestr = (
                        monthdatestr + "," + str(curdate) + getNumberSuffix(curdate)
                    )
            else:
                if curmonth >= 0:
    ...
    for curhour in hourlist:
        for curminute in minutelist:
            if curhour >= 0:
                ...
                hasMinute = True
                hourminstr = (
                    hourminstr + "," + str(curminute) + getNumberSuffix(curminute)
                )
...
def describeAlarm():
    ...
    return (
        describeSchedule([-1], [weekday], [date], [hour], [minute])
        + " Local (RTC Schedule: "
        + utcschedule
        + " UTC)"
    )
...
def getRTCdatetime():
    ...
    try:
        return (
            datetime.datetime(year + 2000, month, date, hour, minute, second)
            + getLocaltimeOffset()
        )
    except:
        return datetime.datetime(2000, 1, 1, 0, 0, 0)
    ...
    try:
        ...
        with open(fname, "r") as fp:
            for curline in fp:
                ...
                tmpline = "".join(
                    [c if 0x20 <= ord(c) and ord(c) <= 0x7E else "" for c in curline]
                )
...
def describeConfigListEntry(configlistitem):
    ...
    return (
        cmd
        + " | "
        + describeSchedule(monthlist, weekdaylist, datelist, hourlist, minutelist)
    )
...
def getCommandForTime(commandschedulelist, datetimeobj, checkcmd):
    ...
    while ctr < len(commandschedulelist):
        ...
        if (testcmd.lower() == checkcmd or len(checkcmd) == 0) and len(testcmd) > 0:
            if (
                checkDateForCommandSchedule(commandschedulelist[ctr], datetimeobj)
                == True
            ):
                return testcmd
        ctr = ctr + 1
    ...
    if addmode == "minute":
        ...
        if testfield < 0:
            if testtime.minute < 59:
                ...
                return incrementCommandScheduleTime(
                    commandschedule, testtime.replace(minute=0), "hour"
                )
        else:
        ...
        if testfield < 0:
            if testtime.hour < 23:
                ...
                return incrementCommandScheduleTime(
                    commandschedule, testtime.replace(hour=0), "date"
                )
        else:
        ...
        if testfield < 0:
            ...
            if testtime.day < maxmonthdate:
                ...
                return incrementCommandScheduleTime(
                    commandschedule, testtime.replace(day=1), "month"
                )
        else:
...
def setNextAlarm(commandschedulelist, prevdatetime):
    ...
    while ctr < len(commandschedulelist):
        ...
        if testcmd == "on":
            ...
            if testweekday >= 0:
                ...
                while (
                    checkDateForCommandSchedule(commandschedulelist[ctr], testtime)
                    == False
                    and (testtime.year - curtime.year) < checklimityears
                ):
    ...
    if foundnextcmd == True:
        # Schedule Alarm
        if (
            nextcommandschedule.get("weekday", -1) >= 0
            or nextcommandschedule.get("date", -1) > 0
        ):
            # Set alarm based on hour/minute of next occurrence to factor in timezone changes if any
            setRTCAlarm(
                True,
                nextcommandschedule.get("weekday", -1),
                nextcommandschedule.get("date", -1),
                nextcommandtime.hour,
                nextcommandtime.minute,
            )
        else:
            ...
            setRTCAlarm(
                ...
                nextcommandschedule.get("weekday", -1),
                nextcommandschedule.get("date", -1),
                nextcommandschedule.get("hour", -1),
                nextcommandschedule.get("minute", -1),
            )
