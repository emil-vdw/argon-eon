...
def argonsysinfo_listcpuusage(sleepsec=1):
    ...
    for cpuname in curusage_a:
        ...
        if curusage_a[cpuname]["total"] == curusage_b[cpuname]["total"]:
            ...
            outputlist.append(
                {"title": cpuname, "value": int(100 * (total - idle) / (total))}
            )
    return outputlist
                    ...
                    if parttype == "ram":
                        ...
                        if lastchar.isdigit() == False:
                            outputlist.append(
                                {
                                    "title": infolist[3],
                                    "value": argonsysinfo_kbstr(int(infolist[2])),
                                }
                            )
                    else:
                        # SD Cards
...
def argonsysinfo_getram():
    ...
    if totalram == 0:
        return "0%"
    return [
        str(int(100 * totalfree / totalram)) + "%",
        str((totalram + 512 * 1024) >> 20) + "GB",
    ]
...
def argonsysinfo_listhddusage():
    ...
    for temp in alllines:
        ...
        if len(infolist) >= 6:
            ...
            if curdev in outputobj:
                outputobj[curdev] = {
                    "used": outputobj[curdev]["used"] + int(infolist[2]),
                    "total": outputobj[curdev]["total"] + int(infolist[1]),
                }
            else:
                outputobj[curdev] = {
                    "used": int(infolist[2]),
                    "total": int(infolist[1]),
                }
        ...
        for temp in alllines:
            ...
            if len(infolist) >= 4:
                ...
                if infolist[0] != "Personalities" and infolist[1] == ":":
                    ...
                    outputlist.append(
                        {"title": devname, "value": raidtype, "info": devdetail}
                    )
