        ...
        if newsetting != "":
            ...
            if current_config_lines[lineidx] != newsetting:
                ...
                new_config = new_config.replace(
                    current_config_lines[lineidx], newsetting, 1
                )
    ...
    if os.path.exists(fw_base + "/aliases/blconfig"):
        with open(fw_base + "/aliases/blconfig", "rb") as f:
            ...
            for d in os.listdir(nvmem_base):
                if os.path.realpath(nvmem_base + d + "/of_node") in os.path.normpath(
                    nvmem_ofnode_path
                ):
                    return (
                        open(nvmem_base + d + "/nvmem", "rb").read().decode("utf-8"),
                        "blconfig device",
                    )
...
class ImageSection:
    def __init__(self, magic, offset, length, filename=""):
        ...
        debug(
            "ImageSection %x offset %d length %d %s" % (magic, offset, length, filename)
        )
...
class BootloaderImage(object):
    ...
    def parse(self):
        ...
        while offset < self._image_size:
            ...
            if magic == 0x0 or magic == 0xFFFFFFFF:
                ...
                raise Exception(
                    "EEPROM is corrupted %x %x %x" % (magic, magic & MAGIC_MASK, MAGIC)
                )
            ...
            if magic == FILE_MAGIC:  # Found a file
                ...
                filename = (
                    self._bytes[offset + 8 : offset + FILE_HDR_LEN]
                    .decode("utf-8")
                    .replace("\0", "")
                )
            debug(
                "section at %d length %d magic %08x %s"
                % (offset, length, magic, filename)
            )
    ...
    def find_file(self, filename):
        ...
        next_offset = (
            self._image_size - ERASE_ALIGN_SIZE
        )  # Don't create padding inside the bootloader scratch page
        ...
        debug(
            "%s offset %d length %d is-last %d next %d"
            % (filename, ret[0], ret[1], ret[2], ret[3])
        )
        return ret
    ...
    def update(self, src_bytes, dst_filename):
        ...
        if hdr_offset + update_len > next_offset:
            raise Exception(
                "Update %d bytes is larger than section size %d"
                % (update_len, next_offset - hdr_offset)
            )
    ...
    def update_file(self, src_filename, dst_filename):
        ...
        if len(src_bytes) > MAX_FILE_SIZE:
            raise Exception(
                "src file %s is too large (%d bytes). The maximum size is %d bytes."
