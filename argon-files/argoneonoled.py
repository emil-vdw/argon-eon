...
def oled_flushblock(xoffset, yoffset):
    ...
    try:
        ...
        bus.write_i2c_block_data(
            ADDR_OLED,
            OLED_SLAVEADDRESS,
            oled_imagebuffer[bufferoffset : (bufferoffset + blocksize)],
        )
    except:
        return
...
def oled_writetext(textdata, x, y, charwd=6, mode=0):
    ...
    try:
        file = open(
            "/etc/argon/oled/font" + str(charht) + "x" + str(charwd) + ".bin", "rb"
        )
    ...
    while ctr < len(textdata):
        ...
        while fontcol < charwd and x < OLED_WD:
            ...
            while fontrow < numfontrow and row < OLED_HT and x >= 0:
                curbit = 0x80
                curbyte = fontbytes[
                    fontoffset + fontcol + (OLED_NUMFONTCHAR * charwd * fontrow)
                ]
                subrow = 0
